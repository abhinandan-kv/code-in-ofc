import { FileTable } from "../models/associationIndex.js";
...
export const getFile = async (req, res) => {
  ...
  try {
    const result = await FileTable.findAll({ where: { userId: id } });

    res.status(200).send({ message: "File Retrived Sucessfully", response: result });
  } catch (error) {
    console.error(error);
