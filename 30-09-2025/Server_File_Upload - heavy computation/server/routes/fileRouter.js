...
import xlsx_comparision from "../utils/xlsx-worker.js";
import { Worker } from "worker_threads";
import xlsxWorker from "../utils/xlsx-worker.js";

XLSX.set_fs(fs);

const fileRouter = Router();

//writing controller here for some weird reason- will change later
...
fileRouter.post("/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (exisitingFile) {
      ...
      xlsx_comparision(exisitingFile.dataValues.filePath, req.file.path);

      function run_xlsx_comparision(n) {
        return new Promise((resolve, reject) => {
          const worker = new Worker("../utils/xlsx-worker.js", {
            workerData: n,
          });
      ...
      console.time("../utils/xlsx-worker.js");
      const mergedWorkbook = await run_xlsx_comparision(exisitingFile.dataValues.filePath, req.file.path);
      console.timeEnd("../utils/xlsx-worker.js");
      console.log(`result =`, mergedWorkbook);

      // const oldWorkbook = await XLSX.readFile(exisitingFile.dataValues.filePath);
      // console.log(oldWorkbook);

      // // const oldDenseWB = await XLSX.read(exisitingFile.dataValues.filePath, { dense: true });                                  // <-- optimization , but still reading to get desired data back
      // // console.log(denseWB);

      // const newWorkbook = await XLSX.readFile(req.file.path);
      // console.log(newWorkbook);

      // // const newDenseWB = await XLSX.read(req.file.path);                                                                       // <-- optimization , but still reading to get desired data back
