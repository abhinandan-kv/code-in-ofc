...
export async function signUp(req, res) {
  ...
  try {
    ...
    const result = await AuthTable.create({ name: name, email: email, password: hashedPassword });
...
export async function loginIn(req, res) {
  ...
  try {
    ...
    const id = userExists.dataValues.id;
    const name = userExists.dataValues.name;
    const token = await jwt.sign({ id, name, email }, process.env.JWT_KEY, { expiresIn: "12h" });
