...
async function createUser({ name, email, password, role, reference, approved }) {
  const existingAdmin = role === "admin" ? await User.findOne({ where: { role: "admin" } }) : null;
...
async function signupAfterSignin(req, res) {
  ...
  try {
    const hashedPassword = await bcrypt.hash(password, SALT_ROUNDS);
    const approved = creatorRole === "admin";
    ...
    const user = await createUser({
      ...
      password: hashedPassword,
      role,
      reference: `Created by ${creatorName}`,
      approved,
    });

    return res.status(201).json({ message: `User created ${approved ? "and approved" : "but pending approval"}` });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ error: err.message });
  }
...
export { userSignUp, signIn, signupAfterSignin, nextVerifyToken, deleteUser };
