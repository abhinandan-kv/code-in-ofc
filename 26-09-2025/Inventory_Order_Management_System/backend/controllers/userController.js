...
export async function login(req, res) {
  ...
  try {
    ...
    if (userDetail.is2Fa == "true") {
      ...
      res.cookie("token", tokenGen, { maxAge: 43200000, httpOnly: true }); //12hour same as jwt expires time
      return res.status(200).json({
...
// //its not forgot passowrd:( mixed logic its changepassword-
...
//       return res
//         .status(403)
//         .send("Incorrect Credentials, check email or password");
//     }
...
export async function forgotPassword(req, res) {
  ...
  try {
    ...
    const tempToken = jwt.sign(
      { id: isUserExists.id, email: isUserExists.email, role: isUserExists.role, name: isUserExists.name },
      process.env.JWT_KEY,
      { expiresIn: "10m" }
    );
    ...
    // const html = `<h1>Welcome to bestEcommerce</h1><p>Click on the link below to reset password</p><button><a href="localhost:5173/forgotpassword/token?${tempToken}">Reset Password</a></button>`;
...
export async function handleEmailToken(req, res) {
  ...
  try {
    ...
    res.status(200).json({ message: "Updated Successfully", role });
  } catch (err) {
    console.error(err);
    res.status(500).json(err);
  }
...
export async function twoFaOtpverification(req, res) {
  ...
  try {
    ...
    const otpExpirationTimeInMS = otpCreatedAtTimeInMS + 600000;
  ...
  try {
    ...
    if (product.vendorProductStock < quantity) {
      return res.status(400).json({
        message: `Only ${product.vendorProductStock} items left in stock`,
      });
    }
...
export async function getUserOrders(req, res) {
  ...
  try {
    const orders = await UserVendorOrder.findAll({
      where: { userId: id },
...
export async function twoFAStatus(req, res) {
  ...
  try {
    const result = await UserTable.findByPk(id);
