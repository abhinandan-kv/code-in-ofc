...
import VarientProduct from "../models/varientProductModel.js";
...
export async function addNewProduct(req, res) {
  ...
  const __filename = url.fileURLToPath(import.meta.url);
  const __dirname = path.dirname(__filename);
  const uploadPath = path.join(__dirname, "..", "..", "uploads", `vendor${vendorId}`, "products");
  ...
  try {
    ...
    //
    const imagePaths = req.files.map((file) => {
      return `vendor${vendorId}/products/${file.filename}`;
    });
...
export async function listAllUserOrders(req, res) {
  ...
  try {
    const allPendingOrders = await UserVendorOrder.findAll({
      where: { vendorId: id },
    });
    ...
    res.status(200).send({
      message: "Pending Orders Retrived Successfully",
      response: allPendingOrders,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json(err);
  ...
  try {
    ...
    if (isOrderBelongsToVendor.vendorId != id) {
      return res.status(403).json({
        message: "Unauthorized to change others order status except yours.",
      });
    }
    // console.log("isOrderBelongsToVendor", isOrderBelongsToVendor);

    //if order is already delivered status cant be changed
    if (isOrderBelongsToVendor.currentStatus == "Delivered") {
      return res.status(200).send({ message: "Order has been already delivered, cant change status now!" });
    } else if ((status = "Delivered")) {
      const changeOrderStatus = await UserVendorOrder.update({ currentStatus: status, isDelivered: true }, { where: { id: orderId, vendorId: id } });
      ...
      return res.status(200).json({ message: "Order status Updated Successfully", updatedStatus });
    } else {
...
export async function totalRevenueMade(req, res) {
  ...
  try {
    ...
    const totalRevenue = await UserVendorOrder.findAll(
      // { attributes: ["productTotalPrice", "userId"] },
    ...
    res.status(200).json({
      message: "Total Revenue information received",
...
// export async function getSubCatgory(req, res) {
//   const id = req.params.id;

//   try {
//     const subCategory = await SubCategory.findAll({ where: { categoryId: id } });
...
export async function addVarient(req, res) {
  ...
  //expecting productid from user
  // const { productId } = req.body;

  try {
    const result = await VarientProduct.create("white", "138gb", "large", "floral pattern");
    console.log(result);
  } catch (err) {
    console.error(err);
