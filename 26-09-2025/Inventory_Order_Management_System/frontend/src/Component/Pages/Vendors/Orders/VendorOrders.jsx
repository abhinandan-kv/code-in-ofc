...
console.log("Order", orders)
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    ...
    // setLoading(false);
    const newData = await axios.get("http://localhost:9000/vendor/allorders", { withCredentials: true });
    setOrders(newData.data.response);
  };
          ...
          {orders.map((order) => (
            <div key={order.id} className="bg-gray-300  p-4 rounded-lg shadow flex flex-col md:flex-row justify-between items-start md:items-center">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Quantity: {order.productQualitity} | Total: ${order.productTotalPrice}
                </p>
