...
export default function ChatPageNew() {
  const [username, setUsername] = useState(() => {
    const result = localStorage.getItem("curr_user_detail");
    const parsedResult = JSON.parse(result);
    return result ? parsedResult.name : null;
  });
  const [room, setRoom] = useState("");
  const [joined, setJoined] = useState(false); // change this to handle room number
  ...
  const [userData, setUserData] = useState(() => {
    const result = localStorage.getItem("curr_user_detail");
    return result ? JSON.parse(result) : { name: null, email: null };
  });
  ...
  const joinChat = (e) => {
    ...
    if (!username) return;
    socket.connect();
    socket.emit("join", { username: username, room: room || null });
    setJoined(true);
  };
