...
export async function updateProduct(req, res) {
  ...
  //  JSON.stringify(Object.fromEntries(req.body));      handle later if using form-data
  ...
  try {
    ...
    auditFunctionality(
      ...
      vendorRole,
      vendorEmail,
      `updated a product, from ${oldData} to ${updatedResult} , productId:${addNewProductFromVendor.id}`
    );
...
export async function listAllUserOrders(req, res) {
  ...
  try {
    ...
    res.status(200).json({ message: "Pending Orders Retrived Successfully", response: allPendingOrders });
  } catch (err) {
    console.error(err);
    res.status(500).json(err);
  }
