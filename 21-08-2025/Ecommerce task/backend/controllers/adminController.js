...
export async function forceListAllProduct(req, res) {
  ...
  try {
    ...
    res.status(200).json({ message: "All records retrived Successfully", records: allRecords });
  } catch (err) {
    console.error(err);
    res.status(500).json(err);
  }
...
export async function readAuditLogs(req, res) {
  ...
  try {
    ...
    res.status(200).json({ message: "Logs retrieved successfully", gettingAllLogs });
  } catch (err) {
    console.error(err);
    res.status(500).json(err);
  }
