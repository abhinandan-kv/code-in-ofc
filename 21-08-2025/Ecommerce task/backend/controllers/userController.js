...
export async function login(req, res) {
  ...
  try {
    ...
    if (userDetail.is2Fa == "true") {
      ...
      return res.status(200).json({ message: "Login Successfully", token: tokenGen });
    }
  } catch (err) {
    console.error(err);
    res.status(500).json(err);
...
export async function userRemoveItemToCard(req, res) {
  ...
  try {
    ...
    const removeFromCart = await UserProductCart.destroy({
      where: { userId: id, productId: productIdToRemove },
    });
    ...
    auditFunctionality(id, name, role, email, `user removed a product from cart productId:${productIdToAdd}`);
...
export async function userSingleOrderPlaced(req, res) {
  ...
  const quantity = parseInt(req.params.id);
